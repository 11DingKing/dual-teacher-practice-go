...
func (r Evidences) Add(ctx context.Context, e domain.Evidence) error {
	_, x := r.DB.ExecContext(writeContext(ctx), "INSERT INTO evidences(id,application_id,kind,title,uri,verification_status,submitted_by,created_at) VALUES(?,?,?,?,?,?,?,?)", e.ID, e.ApplicationID, e.Kind, e.Title, e.URI, e.Status, e.SubmittedBy, e.CreatedAt.Format(time.RFC3339Nano))
	return x
}
func (r Evidences) SetStatus(ctx context.Context, id string, from, to domain.EvidenceStatus, actor string, at time.Time, note string) error {
	res, e := r.DB.ExecContext(writeContext(ctx), "UPDATE evidences SET verification_status=?,verified_by=?,verified_at=?,dispute_note=? WHERE id=? AND verification_status=?", to, actor, at.Format(time.RFC3339Nano), note, id, from)
	if e != nil {
		return e
	}
